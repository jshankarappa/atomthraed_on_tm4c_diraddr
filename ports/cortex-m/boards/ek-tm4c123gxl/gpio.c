/** GPIO
 *
 * The GPIO API provides functionality for accessing the GPIO pins of the TM4C
 *
 * Attention : An important aspect to consider is that libopencm3 uses the
 ...
 * GPIO ports must be enabled via a call to gpio_enable_ahb_aperture().
 * Failing to do so will cause a hard fault.
 *
 * note :
 * Once the AHB aperture is enabled, GPIO registers can no longer be accessed
 * via the APB aperture. The two aperture are mutually exclusive.
 *
 ...
 *
 * Configuring GPIO pins as inputs is done with gpio_mode_setup(), with
 * GPIO_MODE_INPUT for the mode parameter. The direction of the pull-up
 ...
 * gpio_mode_setup(), with GPIO_MODE_ANALOG for the mode parameter.
 *
 ...
 * part's datasheet, and usually varies between parts of the same family.
 *
 * Multiplexing a pin, or group of pins to an alternate function is done with
 * gpio_set_af(). Because AF0 is not used on the LM4F, passing 0 as the
 ...
 *
 * On the TM4C, the NMI and JTAG/SWD default to their alternate function. These
 ...
 * via gpio_unlock_commit. Once a special function pin is unlocked, its
 ...
 *
 * All GPIO registers are accessed via the AHB aperture. It
 * provides faster control over the older APB aperture. This aperture must be
 * enabled before calling any other gpio_*() functions.
 *
 **/
...
void gpio_unlock_commit(uint32_t gpioport, uint8_t gpios)
{
	...
	GPIO_LOCK(gpioport) = GPIO_LOCK_KEY;
 ...
 *
 * Writing a group of pins can be accomplished with gpio_write(). The mask
 ...
 * port.
 *
 * Two extra functions are provided, gpio_port_read() and
 * gpio_port_write(). They are functionally identical to
 * gpio_read (port, GPIO_ALL) and gpio_write (port, GPIO_ALL, val)
 ...
 *
 * note:
 * The gpio_toggle() operation is the only GPIO port operation which is not
 ...
 * NVIC with nvic_enable_irq(). For this last step, the nvic.h header is needed:
 *
