 ...
 * RCC
 *
 * TM4C123GXL Clock control API
 *
 * The TM4C clock API provides functionality for manipulating the system clock,
...
uint32_t tm4c_rcc_sysclk_freq = 16000000;
 ...
 *
 * Set the binary divisor used to pre-divide the system clock down for use as the
...
void rcc_change_pll_divisor(uint8_t pll_div400)
{
	...
	tm4c_rcc_sysclk_freq = (uint32_t)400E6 / pll_div400;
}
...
void rcc_sysclk_config(enum osc_src src, enum xtal_t xtal, uint8_t pll_div400)
{
	...
	if (pll_div400) {
		...
		switch (src) {
		case OSCSRC_PIOSC:
			tm4c_rcc_sysclk_freq = 16000000;
			break;
		case OSCSRC_PIOSC_D4:
		    tm4c_rcc_sysclk_freq = 4000000;
			break;
		case OSCSRC_MOSC:
		    tm4c_rcc_sysclk_freq = xtal_to_freq(xtal);
			break;
		case OSCSRC_32K_EXT:
		    tm4c_rcc_sysclk_freq = 32768;
			break;
